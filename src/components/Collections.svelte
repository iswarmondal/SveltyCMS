...
<script lang="ts">
	...
	import { collection, contentStructure, setMode } from '@stores/collectionStore.svelte';
	...
	// Derived UI & data
	let isFullSidebar = $derived(uiStore.uiVisibility.leftSidebar === 'full');
	let currentLanguage = $derived(contentLanguage.value);
	let selectedId = $derived(collection.value?._id ?? null);
	let activeWidgetList = $derived(activeWidgets);
