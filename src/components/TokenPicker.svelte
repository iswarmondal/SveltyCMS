...
<script lang="ts">
	...
	import { uiVisibility } from '@src/stores/UIStore.svelte';
	...
	let rightPosition = $derived(uiVisibility.current.rightSidebar !== 'hidden' ? '340px' : '2rem');
	...
	$effect(() => {
		...
		const [tokenPath, ...modParts] = match[1].split('|').map((s) => s.trim());
		...
		if (foundToken) {
			...
			selectedModifiers = modParts
				.map((modStr) => {
					...
					const rawArgs = m[2] ? m[2].split(',').map((s) => s.trim().replace(/^['"]|['"]$/g, '')) : [];
	...
	$effect(() => {
		...
		const active = activeInputStore.value;
	...
	>
		<header use:draggable class="mb-4 flex cursor-move select-none items-center justify-between">
			<div class="flex items-center gap-3">
				{#if mode === 'configure'}
					<button onclick={back} class="btn-icon btn-icon-sm variant-ghost-surface" aria-label="Back">
				...
				<h3 class="text-lg font-bold text-tertiary-500 dark:text-primary-500">
					...
					<span class="badge variant-soft-secondary">
						{activeInputStore.value?.field.label || activeInputStore.value?.field.name || 'Field'}
					</span>
				</h3>
			</div>
			<button onclick={() => activeInputStore.set(null)} class="btn-icon btn-icon-sm variant-ghost-surface" aria-label="Close">
						...
						{#if openCategories[cat] || search}
							<div transition:slide class="mt-2 space-y-1">
								{#each tokens as t}
									...
									<div
										class="card variant-filled-surface hover:variant-soft-primary cursor-pointer p-2 transition-colors"
		...
		{:else if selectedToken}
			<div class="flex-1 space-y-4 overflow-y-auto pr-2">
				...
				<div class="card variant-soft-primary border border-primary-500/30 p-4">
					<div class="mb-2 flex items-center justify-between">
						...
						<span class="badge variant-filled-primary">{selectedToken.type}</span>
					</div>
							...
							<div class="card variant-ringed-surface group relative p-3">
								<div class="mb-2 flex items-center justify-between">
									...
									<button
										...
										class="btn-icon btn-icon-sm variant-ghost-error text-error-500"
				...
				<div>
					...
					<div class="flex flex-wrap gap-2">
						{#each availableModifiers as m}
							<button onclick={() => addModifier(m)} class="chip variant-filled-surface hover:variant-filled-secondary transition-colors">
			...
			<div class="mt-4 space-y-3 border-t border-surface-500/30 pt-4">
				...
				<div>
					...
					{#if isLoadingPreview}
						<div class="card variant-soft animate-pulse p-3 text-sm">Resolving...</div>
					{:else}
				...
				<div class="flex gap-2">
					...
					<button onclick={addAnotherToken} class="btn variant-soft-surface flex-1">
						<iconify-icon icon="mdi:plus"></iconify-icon>
						Add Another
					</button>
					<button onclick={insert} class="btn variant-filled-primary flex-1 font-bold"> Insert Token </button>
				</div>
			</div>
		{/if}
	</div>
