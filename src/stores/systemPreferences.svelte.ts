...
export const systemPreferences = {
	...
	setPreferences: (p: DashboardWidgetConfig[]) => preferences.set(p),
	updateWidget: (w: DashboardWidgetConfig) => preferences.updateWidget(w),
	updateWidgets: (w: DashboardWidgetConfig[]) => preferences.updateWidgets(w),
	removeWidget: (id: string) => preferences.removeWidget(id),

	// Svelte store contract for $-prefix usage
	subscribe(fn: (v: DashboardWidgetConfig[]) => void) {
		return $effect.root(() => {
			$effect(() => fn(preferences.preferences));
		});
	}
};
