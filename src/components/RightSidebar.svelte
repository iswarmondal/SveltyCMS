...
<script lang="ts">
	...
	import { cloneCurrentEntry, deleteCurrentEntry, saveEntry } from '../utils/entryActions.svelte';
	...
	import { forceUpdate, uiStore } from '@stores/UIStore.svelte';
	...
	let currentScreenSize = $derived(screenSizeStore.screenSize);
	...
	let shouldDisableStatusToggle = $derived(
		(currentMode === 'create' && !uiStore.isRightSidebarVisible) ||
			(currentMode === 'edit' && !uiStore.isRightSidebarVisible && currentScreenSize !== 'LG') ||
	...
	$effect(() => {
		const unsub = saveLayerStore.subscribe((action) => {
			nextAction = action;
			shouldShowNextButton.set(!!action);
		});
		return unsub;
	});
...
{#if showSidebar}
	<div class="flex h-full flex-col justify-between px-3 py-4">
		...
		{#if $shouldShowNextButton && currentMode === 'create' && isMenuCollection}
